...
module npuarc_uitlb (
  ...
  input [2:0]    alloc_entries,  // Allocatable entries
  ...
  output reg [`npuarc_ITLB_INDEX_RANGE]   rslt0_match_index,
  output reg                       rslt0_match_size,
  output reg                       rslt0_multiple_match,

  output reg [`npuarc_ITLB_ENTRIES_RANGE] rslt0_entry_match,
  output reg [`npuarc_ITLB_ENTRIES_RANGE] rslt0_entry_match_1h,
  ...
  input  [`npuarc_ITLB_ENTRIES_RANGE]     invalidate_entries,
  ...
  output     [`npuarc_ITLB_ENTRIES_RANGE] inval_entry_1h,

  ////////// Interface to read / write utlb entries /////////////////////////
  //
  input                          entry_rd_val,   // Valid read request
  input      [`npuarc_ITLB_INDEX_RANGE] entry_rd_addr,  // index for read
  output reg [`npuarc_UTLB_ENTRY_RANGE] entry_rd_data,  // LR read data (entry)

  // Update / aux write operation from jtlb
  input  [`npuarc_ITLB_ENTRIES_RANGE]   entry_update_1h, // may be more than 1 bit set
...
//
  ...
  reg [`npuarc_ITLB_ENTRIES_RANGE] rslt0_vpn_match;
  reg [`npuarc_ITLB_ENTRIES_RANGE] rslt0_pid_match;
// leda NTL_CON12A off
// leda NTL_CON13A off
// LMD: undriven internal net Range:1-7
// LJ:  all bits are driven
  reg [`npuarc_ITLB_ENTRIES_RANGE] rslt0_entry_match_raw_1h;
// leda NTL_CON13A on
// leda NTL_CON12A on
  ...
  reg [`npuarc_ITLB_ENTRIES_RANGE] rslt_sasid_match;

//`for (4 = 0; 4 < 4; 4++)
//  reg [`PD0_VPN_RANGE]      vpn_mask4_r;
//`endfor

  // make one-hot select bus for reading addressed entry
  wire [`npuarc_ITLB_ENTRIES_RANGE] entry_rd_sel_1h;
  assign entry_rd_sel_1h = 4'h1 << entry_rd_addr;

  wire [`npuarc_ITLB_ENTRIES_RANGE] alloc_entry_mask;
  assign alloc_entry_mask = ~({4{1'b1}} << alloc_entries);

  reg [`npuarc_ITLB_ENTRIES_RANGE]  invalid_entries;
...
// Find lowest set bit (one-hot result)
  ...
  rslt0_entry_match_1h = {4{rslt0_lkup_valid}}
...
// Find lowest set bit (one-hot result)
//
function automatic [`npuarc_ITLB_ENTRIES_RANGE] find_first_set_1h (
  input [`npuarc_ITLB_ENTRIES_RANGE] vec );
  reg found_one;
  integer i;
  begin
    found_one = 1'b0;
    find_first_set_1h = 4'd0;
    for (i=0; i<4; i=i+1)
    begin
...
//
function automatic [`npuarc_ITLB_INDEX_RANGE] find_first_set (
  input [`npuarc_ITLB_ENTRIES_RANGE] vec );
  reg found_one;
  integer i;
  begin
