...
module npuarc_wd_parity #(parameter WD_REG_WIDTH=32) (
...
////////// General input signals ///////////////////////////////////////////
  ...
  ////// Outputs /////////////////////////////////////////////
    ...
    output reg      watchdog_parity_err , //Additional bit for error output
 ...
 always @*
 begin : d_parity_PROC
  d_parity  = d_in[1] ^ d_in[0];             
    for (i=2;i<WD_REG_WIDTH;i=i+1)
      d_parity = d_parity ^ d_in[i];          

  q_parity_chk  = q_out[1] ^ q_out[0];   
             
    for (i=2;i<WD_REG_WIDTH;i=i+1)
      q_parity_chk = q_parity_chk ^q_out[i];          
 
  watchdog_parity_err = (q_parity_chk != q_parity) ? 1'b1 : 1'b0; 
  end
