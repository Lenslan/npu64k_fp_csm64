...
module npuarc_alb_ifu_data_mux_fifo 
  ( 
  input                    clk,
  input                    rst_a,
  input                    restart,
  ...
  input [88:0]     wdata,
  input                        rd,
  output reg [88:0]    rdata,
  output [3:0] rdata_flop,
// leda NTL_CON32 off
...
begin: fifo_mem_rd_nxt_PROC
  if (wr && (cnt_nxt == { {2{1'b0}}, 1'b1 })) begin //bypass input
    rdata_r_nxt = wdata[3: 0]; 
  end
  else begin
    rdata_r_nxt = rdata_erly[3: 0]; 
  end
end //block: fifo_mem_PROC
