...
// Target part of AXI async bridge
...
module npu_axi_async_tgt
  ...
  (
   ...
   output logic clk_req_a,
   // AXI master interface
   `AXIMST(AXIIDW,AXIDW,AXIAWUW,AXIWUW,AXIBUW,AXIARUW,AXIRUW,axi_mst_),
   // AXI slave async interface
   `AXIASYNCSLV(AXIIDW,AXIDW,AXIAWUW,AXIWUW,AXIBUW,AXIARUW,AXIRUW,AWFIFO_DEPTHL2,WFIFO_DEPTHL2,BFIFO_DEPTHL2,ARFIFO_DEPTHL2,RFIFO_DEPTHL2,axi_async_slv_)
   );
  ...
  // simple assignments
  //
  assign clk_req_a       = axi_async_slv_clk_req_a | clk_req_r;
  ...
  (
   ...
   .rdpnt         ( axi_async_slv_aw_rdpnt  ),
   .rdata         ( axi_async_slv_aw_data   ),
   .wpnt_a        ( axi_async_slv_aw_wpnt_a ),
   .rpnt          ( axi_async_slv_aw_rpnt   )
  );
  ...
  (
   ...
   .rdpnt         ( axi_async_slv_w_rdpnt  ),
   .rdata         ( axi_async_slv_w_data   ),
   .wpnt_a        ( axi_async_slv_w_wpnt_a ),
   .rpnt          ( axi_async_slv_w_rpnt   )
  );
  ...
  (
   ...
   .rdpnt          ( axi_async_slv_b_rdpnt  ),
   .rdata          ( axi_async_slv_b_data   ),
   .wpnt           ( axi_async_slv_b_wpnt   ),
   .rpnt_a         ( axi_async_slv_b_rpnt_a )
  );
  ...
  (
   ...
   .rdpnt         ( axi_async_slv_ar_rdpnt  ),
   .rdata         ( axi_async_slv_ar_data   ),
   .wpnt_a        ( axi_async_slv_ar_wpnt_a ),
   .rpnt          ( axi_async_slv_ar_rpnt   )
  );
  ...
  (
   ...
   .rdpnt          ( axi_async_slv_r_rdpnt  ),
   .rdata          ( axi_async_slv_r_data   ),
   .wpnt           ( axi_async_slv_r_wpnt   ),
   .rpnt_a         ( axi_async_slv_r_rpnt_a )
  );

endmodule : npu_axi_async_tgt
