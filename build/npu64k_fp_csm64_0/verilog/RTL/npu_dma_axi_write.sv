...
module npu_dma_axi_write
  import npu_types::*;
  #(
    // AXI data with XM
    parameter AXI_MST_IDW = 1,
    parameter DATA_LEN = 6,
    parameter BUFFER_SIZE = 1024, //MAX BUFFER_SIZE is 1024,
...
// spyglass enable_block W240
  ...
  localparam int MAX_BSIZE     = BUFFER_SIZE/2;
  ...
  localparam int AXI_SIZE      = $clog2(VSIZE)+3;
...
// spyglass enable_block W116
  ...
  // bresp entries
  logic [AXI_OST-1:0]              bresp_r;
  logic [AXI_OST-1:0]              bresp_nxt;
  logic                            bresp_en;

  logic [ENTRIES_WIDTH-1:0]        bresp_wr_ptr_r;
  logic [ENTRIES_WIDTH-1:0]        bresp_wr_ptr_nxt;
  logic                            bresp_wr_ptr_en;
  ...
  logic [FUL_WLEN-1:0]             dma_wop_nxt_lent;

  
  // Command & Package Update
// spyglass disable_block SelfDeterminedExpr-ML W362
//SMD:Self determined and width mismatch
//SJ :Ignore width mismatch (buffer_vld_size - buffer_alloc_r) > (xm_cmd_r.xm_bcmd.len - xm_cmd_r.shamt)
//generate
...
//      if (((wop_rd_ptr_r ^ wop_wr_ptr_r) != {1'b1, {ENTRIES_WIDTH{1'b0}}}) && // WOP entries not full
//          (bresp_r[wop_wr_ptr_r[ENTRIES_WIDTH-1:0]] == 1'b0) && //Bresp is already released in target entry
...
generate
if(FUL_WLEN > AXI_WLEN) begin : PROC_WOP_AXI_GT_LENT
  always_comb
  begin : dma_wop_axi_lent_GT_LENT_PROC
     dma_wop_nxt_lent    = xm_boost_r ? xm_cmd_r.xm_bcmd.len[AXI_ALSB+:FUL_WLEN] : {{(FUL_WLEN-AXI_WLEN){1'b0}}, xm_cmd_r.xm_bcmd.len[AXI_ALSB+:AXI_WLEN]};
  end
end else begin:  PROC_WOP_AXI_NGT_LENT
  always_comb
  begin : dma_wop_axi_lent_NGT_LENT_PROC
     dma_wop_nxt_lent    = xm_cmd_r.xm_bcmd.len[AXI_ALSB+:FUL_WLEN];
  end
end
endgenerate
  ...
  begin : dma_wop_axi_PROC
    ...
    dma_daxi_aw.size       = AXI_SIZE;   // 512b data
    dma_daxi_aw.region     = 2'd0;   
    dma_daxi_wvalid        = 1'b0;
    dma_daxi_wdata         = 'b0;
    dma_daxi_wstrb         = 'b0;
    dma_daxi_wlast         = lst_beat;
    dma_daxi_bready        = 1'b1;   // always accept bresp
    `ifdef SLICE_AXI_UITF
    dma_daxi_awuser        = '0;
// spyglass disable_block W164b
// SMD: LHS large than RHS
// SJ: Has default value, and AXI write only need VMID, no BDMA flags required
...
// spyglass enable_block W164b
    ...
    bresp_en               = 1'b0;
    bresp_nxt              = bresp_r;
    bresp_wr_ptr_en        = 1'b0;
    bresp_wr_ptr_nxt       = bresp_wr_ptr_r;
    ...
    // Initialize issue
    if ((npu_axi_state_r == npu_axi_status_idle_e) && (xm_w_valid)) begin
...
// spyglass enable_block SelfDeterminedExpr-ML W362
      begin
        ...
        if (dma_daxi_awready == 1'b1) begin
          ...
          wop_nxt[wop_wr_ptr_r[ENTRIES_WIDTH-1:0]].lent    = dma_wop_nxt_lent;
          //if (FUL_WLEN > AXI_WLEN) begin
          //  wop_nxt[wop_wr_ptr_r[ENTRIES_WIDTH-1:0]].lent    = xm_boost_r ? xm_cmd_r.xm_bcmd.len[AXI_ALSB+:FUL_WLEN] : {{(FUL_WLEN-AXI_WLEN){1'b0}}, xm_cmd_r.xm_bcmd.len[AXI_ALSB+:AXI_WLEN]};
          //end
          //else begin
...
// spyglass enable_block W362
      ...
      dma_daxi_wdata      =  buffer_rd_data & axi_wdata_mask;
      ...
      if (dma_daxi_wready) begin
        ...
        if (lst_beat == 1'b1) begin
          ...
          bresp_en                = 1'b1;
          bresp_nxt[wop_rd_ptr_r[ENTRIES_WIDTH-1:0]] = 1'b1;
        end
        else begin
...
// spyglass enable_block W116
    ...
    if (dma_daxi_bvalid == 1'b1) begin
      bresp_en                  = 1'b1;
      bresp_nxt[bresp_wr_ptr_r] = 1'b0;
      xm_err_en                 = 1'b1;
      xm_err_nxt                = ((dma_daxi_bresp != npu_axi_resp_okay_e) && (dma_daxi_bresp != npu_axi_resp_exokay_e));
      bresp_wr_ptr_en           = 1'b1;
      bresp_wr_ptr_nxt          = bresp_wr_ptr_r + 1;
    end
  ...
  begin : reg_pipe_PROC
    ...
    begin
      ...
      if (bresp_en)
        bresp_r <= bresp_nxt;
      if (bresp_wr_ptr_en)
        bresp_wr_ptr_r <= bresp_wr_ptr_nxt;
    end
  end : reg_pipe_PROC

  // Assigns
  assign idle     = ((npu_axi_state_r == npu_axi_status_idle_e) && (wop_rd_ptr_r == wop_wr_ptr_r) && (bresp_r[bresp_wr_ptr_r] == 1'b0));
  assign lst_beat = (wlent_r == wop_r[wop_rd_ptr_r[ENTRIES_WIDTH-1:0]].lent) ? 1'b1 : 1'b0;
  assign err_st   = xm_err_r;
endmodule : npu_dma_axi_write
