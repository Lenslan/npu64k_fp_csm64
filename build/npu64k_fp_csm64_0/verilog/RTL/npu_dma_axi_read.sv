...
module npu_dma_axi_read
  ...
  #(
    ...
    parameter MAXIRUW         = 1, 
    parameter STU_D    = DMA_VM_LANES_L*8*ISIZE,
    parameter AD_WID   = $clog2(DMA_VM_LANES_L*ISIZE),
    parameter BUFFER_SIZE = 128*VSIZE
   )
  (
   // interfaces
   //
...
// spyglass enable_block W240
   ...
   input  logic                    boost,
   input  logic  [1:0]             rd_ost,
   input  logic  [7:0]             bc_flags,
   
   output logic                    buffer_wr_valid, 
   output logic  [AD_WID-1:0]      buffer_wr_num, 
   output logic  [AD_WID-1:0]      buffer_wr_alsb, 
   output logic  [STU_D-1:0]       buffer_wr_data, 
...
// SJ: Has default value, and RHS is the real size
    if (FUL_RLEN > AXI_RLEN) begin
      ...
      arlenb  =  {(~xm_cmd_in.base[AXI_ALSB+:FUL_RLEN]),pad_msk};
    end
...
// spyglass enable_block W116
  ...
  begin : dma_ptt_axi_PROC
    ...
    dma_saxi_ar.size    = AXI_SIZE;
    dma_saxi_ar.region  = 2'b0;
    ptt_tail_en         = 1'b0;
...
//SJ :Ignore buffer_free_r width mismatch with xm_cmd_r.xm_bcmd.len, no influence on function
      if ((((ptt_tail_r ^ ptt_head_r) & ptt_mask) != ptt_full) && // PTT not full 
          (xm_boost_r || (buffer_free_r >= xm_cmd_r.xm_bcmd.len)) // Buffer available check (Skip in boost mode)
         )
...
//SJ :Ignore buffer_free_r width mismatch with xm_cmd_r.xm_bcmd.len, no overflow
          buffer_free_nxt     = buffer_free_nxt - xm_cmd_r.xm_bcmd.len;
// spyglass enable_block W116
          buffer_free_en      = 'b1;
        end
      end
...
//SMD:Expression width mismatch (-)
//SJ :Ignore buffer_free_r width mismatch with xm_cmd_r.xm_bcmd.len, no overflow
          buffer_free_nxt    = buffer_free_nxt + ptt_r[ptt_head_r[PTT_WIDTH-1:0]].lent;
// spyglass enable_block W116
          buffer_free_en     = 'b1;
        end
      end
