...
// LJ: this module contains special checker code that should be excepted from Synthesis and Leda checks
  ...
  // Only in the 3rd cycle data_out gets the value of data_in.
  logic [WIDTH-1:0]  data_int;
  assign data_int = (~checker_out_ctrl) & (~$stable(data_in,@(posedge clk))) ? 'x : data_in;
  assign data_out = (~checker_out_ctrl) & (~$stable(data_int,@(posedge clk))) ? 'x : data_int;

  stable_check: assert property (@(posedge clk) disable iff (rst_a !== 1'b0) valid & ~$stable(data_in) |=> ##[1:2] $stable(data_in))
    else      $error("[ERROR - %m]: Violation found \n");
 // leda on
 `endif
`endif

endmodule : npu_3cyc_checker
