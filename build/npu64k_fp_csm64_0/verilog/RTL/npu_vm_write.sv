...
//
// Parameterizable VM write interface
// vcs -sverilog npu_types.sv npu_iterator.sv npu_agu.sv npu_vm_write.sv
// analyze -format sverilog [list ../npu_types.sv ../npu_iterator.sv ../npu_vm_write.sv]
//
...
module npu_vm_write
  ...
  (
   ...
   input  logic      [ISIZE-1:0]   cmask,               // per channel write mask
  ...
  (
   .clk        ( clk            ),
   .rst_a      ( rst_a          ),
   .soft_rst   ( 1'b0           ), // intentionally tied to 0, not soft reset
   .in_valid   ( in_valid       ),
   .in_accept  ( in_accept      ),
   .iter       ( iter           ),
   .offset     ( offset         ),
