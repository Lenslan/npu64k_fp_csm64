...
reg [3:0] i_tap_state_nxt;      // TAP controller state signal
...
always @(posedge jtag_tck or negedge jtag_trst_n)
begin : THE_TAP_CONTROLLER_REG_PROC
  if (jtag_trst_n == 1'b0)
    i_tap_state_r <= S_TEST_LOGIC_RESET;
  else
    i_tap_state_r <= i_tap_state_nxt;
end // block : THE_TAP_CONTROLLER_REG_PROC
...
begin : THE_TAP_CONTROLLER_PROC
        ...
        S_TEST_LOGIC_RESET :
        if (jtag_tms == 1'b0)
                i_tap_state_nxt = S_RUN_TEST_IDLE;
        else
                i_tap_state_nxt = S_TEST_LOGIC_RESET;
        ...
        if (jtag_tms == 1'b0)
                ...
                i_tap_state_nxt = S_SELECT_DR_SCAN;
        ...
        S_SELECT_DR_SCAN :
        if (jtag_tms == 1'b0)
                i_tap_state_nxt = S_CAPTURE_DR;
        else
                i_tap_state_nxt = S_SELECT_IR_SCAN;
        ...
        S_CAPTURE_DR :
        if (jtag_tms == 1'b0)
                i_tap_state_nxt = S_SHIFT_DR;
        else
                i_tap_state_nxt = S_EXIT1_DR;
        ...
        S_EXIT1_DR :
        if (jtag_tms == 1'b0)
                i_tap_state_nxt = S_PAUSE_DR;
        else
                i_tap_state_nxt = S_UPDATE_DR;
        ...
        S_CAPTURE_IR :
        if (jtag_tms == 1'b0)
                i_tap_state_nxt = S_SHIFT_IR;
        else
                i_tap_state_nxt = S_EXIT1_IR;
        ...
        S_EXIT1_IR :
        if (jtag_tms == 1'b0)
                i_tap_state_nxt = S_PAUSE_IR;
        else
                i_tap_state_nxt = S_UPDATE_IR;
...
assign shft2exit1_dr_nxt  = ( i_tap_state_r == S_SHIFT_DR ) &&
                            ( jtag_tms == 1'b1 );
...
endmodule // module npu_tap_controller
