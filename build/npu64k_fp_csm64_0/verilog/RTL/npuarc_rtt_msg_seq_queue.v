...
module npuarc_rtt_msg_seq_queue (
                          rtt_clk,
                          rst_a,
                          wr_en,
                          rd_en,
...
input   [10-1:0]    wr_data;
output  [10-1:0]    rd_data;       

output                         empty;
output                         full;


output reg   [6-1:0]   wr_ptr;
output reg   [6-1:0]   rd_ptr;
...
reg  [10-1:0] msg_seq_order_q [47:0];
...
assign rd_data = rd_en ? msg_seq_order_q[rd_ptr] : 10'b0;
...
begin
  if(rst_a == 1'b1 )
    wr_ptr <= 6'b0;
  else if(wr_en && wr_ptr == 6'd47)
    wr_ptr <= 6'b0;
  else if(wr_en)
    wr_ptr <= wr_ptr_xx[6-1:0];
end
...
begin
  if(rst_a == 1'b1 )
    rd_ptr <= 6'b0;
  else if(rd_en && rd_ptr == 6'd47)
    rd_ptr <= 6'b0;
  else if(rd_en)
    rd_ptr <= rd_ptr_xx[6-1:0];
end
