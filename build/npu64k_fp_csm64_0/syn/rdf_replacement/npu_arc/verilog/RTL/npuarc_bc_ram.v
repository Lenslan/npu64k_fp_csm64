...
module npuarc_bc_ram (
  ...
  input                              clk;
  input [`npuarc_BR_BC_DATA_RANGE]  din;
  input [`npuarc_BR_BC_IDX_RANGE]   addr;
  input                              cs;
  input                              we;
  input   [`npuarc_BR_BC_DATA_RANGE] 	  wem;
  input                              ds;
  input                              sd;
  input 			     ls;
  output [`npuarc_BR_BC_DATA_RANGE] dout;
...
`ifdef SizeBitsExtended
     ...
     wire [`npuarc_BR_BC_DATA_MSB+`SizeBitsExtended:0] dout_extra; 
     wire [`npuarc_BR_BC_DATA_MSB+`SizeBitsExtended:0] din_extra;
     wire [`npuarc_BR_BC_DATA_MSB+`SizeBitsExtended:0] wem_extra;
     assign dout = dout_extra[`npuarc_BR_BC_DATA_MSB:0];
     assign din_extra = {{`SizeBitsExtended{1'b0}},din} ;
     assign wem_extra = {{`SizeBitsExtended{1'b0}},wem} ;
 `else 
  
     wire [`npuarc_BR_BC_DATA_MSB:0] dout_extra; 
     wire [`npuarc_BR_BC_DATA_MSB:0] din_extra;
     wire [`npuarc_BR_BC_DATA_MSB:0] wem_extra;
     assign dout = dout_extra[`npuarc_BR_BC_DATA_MSB:0];
  ...
  `ifdef BldCfgSizeWordsExtended
     wire [`npuarc_BR_BC_IDX_MSB+`BldCfgSizeWordsExtended:`npuarc_BR_BC_IDX_LSB] addr_extra;
     assign addr_extra = {{`BldCfgSizeWordsExtended{1'b0}},addr} ;
  `else
     wire [`npuarc_BR_BC_IDX_MSB:`npuarc_BR_BC_IDX_LSB] addr_extra;
...
//leda NTL_CON12 off
//leda WV951025 off
  `Membc_ram `RAM_WEM_ts07n0g41p11sadcl02ms(bc_ram0,dout_extra,addr_extra,din_extra,wem_extra,we,cs,clk,ds,sd,i_ls)
//leda WV951025 on
//leda NTL_CON12 on
//leda NTL_CON08 on
