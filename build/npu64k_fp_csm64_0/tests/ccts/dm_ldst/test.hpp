...
public:
  ...
  void exec() {
    ...
    if(proc_id == 0) { //L2 ARc
        ...
        #if (!defined(TB_MDB))||(0!=(TB_MDB&0x0f))
        ...
        for(int grp=0; grp<NPU_NUM_GRP; grp++){
          for(int slc=0; slc<NPU_NUM_SLC_PER_GRP; slc++){
            uint32_t base_addr = LOCAL_CLST0_BASE + grp*0x100_0000 + slc * 0x40_0000 + L1_DM_OFFSET;
        #ifndef NPU_MEM_ECC
            switch((grp*NPU_NUM_SLC_PER_GRP + slc)%4) { // granularity size
                case 0 :  mem_boundary_cross_test((char     *)base_addr, mem_size, 32); break;
                case 1 :  mem_boundary_cross_test((short    *)base_addr, mem_size, 32); break;
                case 2 :  mem_boundary_cross_test((uint32_t *)base_addr, mem_size, 32); break;
                case 3 :  mem_boundary_cross_test((long long*)base_addr, mem_size, 32); break;
            }
        #else
        ...
        for(int slc=-1; slc<NPU_SLICE_NUM; slc++){
            if(slc==-1) { //slice access memory via its local perip
                base_addr = LOCAL_PERI_BASE +  L1_DM_OFFSET;
            }else{ //slice access other slice mem via cluster perip
                //base_addr = LOCAL_CLST0_BASE + slc * 0x40_0000 + L1_DM_OFFSET;
            }
    ...
    set_sim_finish_flag(err_cnt,proc_id);
  }
