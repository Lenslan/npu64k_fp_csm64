...
public:
  ...
  void prep_operands() {
    ...
    tensor_t<pix_t,4,buffer_t> xmitns(xmibuf, xmi_fshp);
    tensor_t<pix_t,4,buffer_t> xmitns_cut(xmitns.slice(xmi_pos, xmi_cshp));

    csmalloc->alloc(xmobuf, get_shape_size(xmo_fshp));
    tensor_t<pix_t,4,buffer_t> xmotns(xmobuf, xmo_fshp);
    tensor_t<pix_t,4,buffer_t> xmotns_cut(xmotns.slice(xmo_pos, xmo_cshp));
