...
public:
  ...
  void aot_compile() {
    ...
    tensor_t<coef_t,7,xbuffer_t> citns(cibuf, {1,192,1,1,1,64,1});
    ...
    dummy_alloc->alloc(pibuf,1*192*1);
    tensor_t<pix_t,2,xbuffer_t> pitns(pibuf,{(aoffset_t)1,(aoffset_t)192});
    ...
    do {
        otn.write(cAcc[(otn.get_index(0)+1*1*otn.get_index(2)+2*1*otn.get_index(1)+2*1*60*otn.get_index(4))]);
    } while(otn.next());
  ...
  #ifndef RTL_ARC
    do{
        cAcc[(otn.get_index(0)+1*1*otn.get_index(2)+2*1*otn.get_index(1)+2*1*60*otn.get_index(4))] = otn.read();
    } while(otn.next());
