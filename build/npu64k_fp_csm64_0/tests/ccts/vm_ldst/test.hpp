    ...
    if(proc_id == 0) { //L2 ARc
        ...
        for(int grp=0; grp<NPU_NUM_GRP; grp++){
          for(int slc=0; slc<NPU_NUM_SLC_PER_GRP; slc++){
              uint32_t base_addr = LOCAL_CLST0_BASE + grp*0x100_0000 + slc * 0x40_0000 + L1_VM_OFFSET;
        #ifdef NPU_MEM_ECC
              mem_boundary_cross_test((long long*)base_addr, mem_size, 32); break;
        #else
              switch((grp*NPU_NUM_SLC_PER_GRP + slc)%4) { // granularity size
                  case 0 :  mem_boundary_cross_test((char     *)base_addr, mem_size, 32); break;
                  case 1 :  mem_boundary_cross_test((uint16_t *)base_addr, mem_size, 32); break;
