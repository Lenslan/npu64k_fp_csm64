...
#ifndef TENSOR_API_GTOA_ELTOP_CLIP_HPP_
#define TENSOR_API_GTOA_ELTOP_CLIP_HPP_
...
namespace tensor::v200 {
  ...
  class gtoa_eltop_clip_params : public gtoa_eltop_act_params<B> {
  public:
    //
    // Constructor
    //
    ...
    inline gtoa_eltop_clip_params() = default;
    inline gtoa_eltop_clip_params(const gtoa_eltop_clip_params&)  = default;
    ...
    void param_enc(const tensor_t<prescale_t,1,xbuffer_t>&   prescale0,   // per channel 2b+6b prescale for scaling input accum
                   ...
                   const tensor_t<fp_e8m23_t,1,xbuffer_t>&   clipmax,     // per channel output max quantization offset [Cout]
                   const tensor_t<fp_e8m23_t,1,xbuffer_t>&   clipmin,     // per channel output min quantization offset [Cout]
                   // outputs, buffers preallocated by caller
                   gtoa_params_impl_pchan<xbuffer_t>&        obuf         // output encoded parameters
                   );
  // include legacy interface
#include "./legacy/tensor_gtoa_eltop_clip_legacy.hpp"

#include "tensor_gtoa_eltop_clip_param_priv.hpp"
  };

  // include implementation specific inline functions
#include "tensor_gtoa_eltop_clip_inline.hpp"
}
#endif // TENSOR_API_GTOA_ELTOP_CLIP_HPP_
