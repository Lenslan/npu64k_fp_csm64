...
#ifndef TENSOR_GTOA_ELTOP_ACT_INCLUDED
#define TENSOR_GTOA_ELTOP_ACT_INCLUDED
...
namespace tensor::v200 {
  ...
  class gtoa_eltop_act_params : public gtoa_params<B> {
  public:
    //
    // Constructors
    //
    ...
    inline gtoa_eltop_act_params() = default;
    inline gtoa_eltop_act_params(const gtoa_eltop_act_params&)  = default;
    ...
    void init_l1_input0(
                 const impl_spec_container_t<BD>&      p      // structure with allocated buffers and tensors in L1 memory input feature-map
                 );
    template<template<typename> class BD=B>
    void init_l1_input(
                 const impl_spec_container_t<BD>&      p      // structure with allocated buffers and tensors in L1 memory input feature-map
    ...
    template<template<typename> class BD=B>
    void init_l1_output(
                 const impl_spec_container_t<BD>&      p      // structure with allocated buffers and tensors in L1 memory output feature-map
                 );
    //
    ...
    //
    // Set the brdc_f with the broadcasted flags for each inputs dimensions [w,ch]
    //
    void set_broadcast_flags(broadcast_t bc_flags);

    //
    // Set the H-dimension of the input in AM by 1 in case of broadcasting
    //
    void set_Hdim_AM_by_one(void);

    // include legacy interface
#include "./legacy/tensor_gtoa_eltop_act_legacy.hpp"

  protected:
    // include implementation specific private members
#include "tensor_gtoa_eltop_act_param_priv.hpp"
  };

  // include implementation specific inline functions
#include "tensor_gtoa_eltop_act_inline.hpp"
}
#endif
