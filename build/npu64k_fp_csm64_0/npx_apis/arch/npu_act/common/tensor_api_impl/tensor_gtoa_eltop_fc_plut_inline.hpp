...
/*
 * tensor_gtoa_eltop_fc_plut_inline.hpp
 *
 * File defining a tensor level plut API base on the generic tensor operation API
 *
 */
#ifndef NPU_ACT_COMMON_TENSOR_API_IMPL_TENSOR_GTOA_ELTOP_FC_PLUT_INLINE_HPP_
#define NPU_ACT_COMMON_TENSOR_API_IMPL_TENSOR_GTOA_ELTOP_FC_PLUT_INLINE_HPP_
#include "./legacy/tensor_gtoa_eltop_fc_plut_inline_legacy.hpp"

// constructors
// Fully connected convolution + eltop + plut from accumulator (32b/48b) to feature-map (8b/16b)
template<template<typename> class B>
gtoa_eltop_fc_plut_params<B>::gtoa_eltop_fc_plut_params(
  ...
  gtoa_eltop_fc_act_params<B>::set_per_channel(ELTOP_FC_PLUT_PAR_PER_CHAN);
  poscl = poscli;
}

template<template<typename> class B>
void gtoa_eltop_fc_plut_params<B>::init_prog(act_bin_op_t op, bool ea) {
  // initialize the eltwise FC PLUT microcode
  init_eltop_fc_lut(gtoa_params<B>::hlapi, op, ea, poscl, gtoa_eltop_fc_act_params<B>::fp16scl);
}
...
template<template<typename> class B>
void gtoa_eltop_fc_plut_params<B>::param_enc(
  ...
  init_eltop_fc_plut(gtoa_params<B>::hlapi,
                      gtoa_eltop_fc_act_params<B>::op,
                      gtoa_eltop_fc_act_params<B>::ea_ord,
                      gtoa_eltop_fc_plut_params<B>::poscl,
  ...
  aoffset_t j = 0;
  int ci = (gtoa_eltop_fc_plut_params<B>::cmax+ISIZE-1)/ISIZE;
  for (int c = 0; c < ci; c++) {
    for (int i = 0; i < ISIZE; i++) {
      if (j < gtoa_eltop_fc_plut_params<B>::cmax) {
        int16_t sclpst = ((uint32_t)(post.read({j}).data)) | (((int32_t)asymm.read({j}))<<8);
        bparsh[2*c*ELTOP_FC_PLUT_PAR_PER_CHAN*ISIZE+0*ISIZE+i] = scale.read({j}).data;               // BRH0 optional scale
        bparsh[2*c*ELTOP_FC_PLUT_PAR_PER_CHAN*ISIZE+1*ISIZE+i] = sclpst;                             // BRH1 postscale + offset
        bparsw[1*c*ELTOP_FC_PLUT_PAR_PER_CHAN*ISIZE+1*ISIZE+i] = bias.read({j}).data;                // BRW1 bias
        j++;
      }
    }
  }
...
#endif  // NPU_ACT_COMMON_TENSOR_API_IMPL_TENSOR_GTOA_ELTOP_FC_PLUT_INLINE_HPP_
