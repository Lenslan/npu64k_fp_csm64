...
/*
 * tensor_gtoa_binary_mm_inline.hpp
 *
 * File defining a tensor level binary API base on the generic tensor operation API
 *
 */
#include "./legacy/tensor_gtoa_binary_mm_inline_legacy.hpp"
#define HLAPI gtoa_params<B>::hlapi

// constructors
// floating point extensions
template<template<typename> class B>
gtoa_binary_mm_params<B>::gtoa_binary_mm_params(
...
template<template<typename> class B>
void gtoa_binary_mm_params<B>::init_bin_prog(act_bin_op_t opi, broadcast_t brdc_f) {
  ...
  // initialize the binary_mm microcode
  if (isfp) {
    init_binary_rr_fp(HLAPI, opi);
  }
  else {
    init_binary_rr(HLAPI, opi);
  }
}
