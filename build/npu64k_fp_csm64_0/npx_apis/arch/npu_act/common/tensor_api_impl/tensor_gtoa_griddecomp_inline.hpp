...
/*
 * tensor_gtoa_griddecomp_inline.hpp
 *
 * File defining a tensor level griddecomp API based on the generic tensor operation API
 *
 */

#ifndef NPU_ACT_COMMON_TENSOR_API_IMPL_TENSOR_GTOA_GRIDDECOMP_INLINE_HPP_
#define NPU_ACT_COMMON_TENSOR_API_IMPL_TENSOR_GTOA_GRIDDECOMP_INLINE_HPP_

#define HLAPI gtoa_params<B>::hlapi

// constructor
// griddecomp operation from grid (float 16b) to indexes (int 16b) and fracts (float 16b)
template<template<typename> class B>
gtoa_griddecomp_params<B>::gtoa_griddecomp_params(
  ...
  // in0 index input is bf16/fp16 from VM
  // out is a composition from bf16/fp16 and int16 to VM
  HLAPI.i.u.op.io_en    = ACT_IO_EN_FM0 | ACT_IO_EN_OFM;
  HLAPI.i.u.op.bf       = (1<<ACT_BF_OP_IN0DBL_START) | (1<<ACT_BF_OP_OUTDBL_START);
  switch (idxtp) {
    case dtype_fp16:
      HLAPI.i.u.op.bf  |= ACT_BF_OP_IN0FP16_MASK | ACT_BF_OP_OUTFP16_MASK;
      break;
    case dtype_bf16:
      HLAPI.i.u.op.bf  |= ACT_BF_OP_IN0BF16_MASK | ACT_BF_OP_OUTBF16_MASK;
      break;
  ...
  HLAPI.i.u.op.out.fm.fm_agu.iter[ACT_FM_LOOPS - 2] = 2;
...
void gtoa_griddecomp_params<B>::init_l1_output(
  ...
  // [2][D2][D1/8][D0][2] --> [2][D2][D1/8/wch][D1_wch][D0][2]
  tensor_t<ixpix_t, 6, BD> t0(p.data.split(2, HLAPI.i.u.op.out.fm.fm_agu.iter[ACT_FM_LOOPS - 4]));
  // transpose [1][D2][D1/8/wch][D1_wch][D0][2] to [D1_wch][D2][D1/8/wch][D0][2][2]
  tensor_t<ixpix_t, 6, BD> t1(t0.transpose({0, 5, 1, 3, 4, 2}));
  ...
  HLAPI.i.u.op.out.fm.fm_agu.offsets[ACT_FM_LOOPS - 2] = t1.get_offset_last(1);  // 2 (fract & int)
  HLAPI.i.u.op.out.fm.fm_agu.offsets[ACT_FM_LOOPS - 3] = t1.get_offset_last(2);  // D0
  HLAPI.i.u.op.out.fm.fm_agu.offsets[ACT_FM_LOOPS - 4] = t1.get_offset_last(3);  // D1/8/wch
  HLAPI.i.u.op.out.fm.fm_agu.offsets[ACT_FM_LOOPS - 5] = t1.get_offset_last(4);  // D2
}

// initialize the index input tensor
template <template <typename> class B>
template <template <typename> class BD>
void gtoa_griddecomp_params<B>::init_l1_input0(
...
template<template<typename> class B>
inline void gtoa_griddecomp_params<B>::get_shapes(
...
#endif  // NPU_ACT_COMMON_TENSOR_API_IMPL_TENSOR_GTOA_GRIDDECOMP_INLINE_HPP_
