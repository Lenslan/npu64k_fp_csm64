...
/*
 * tensor_gtoa_relu6_inline_legacy.hpp
 *
...
// 32b input, 8b output
template<template<typename> class B>
void gtoa_relu6_params<B>::param_enc(const tensor_t<int32_t,1,xbuffer_t>&      bias,        // per channel bias [Cout]
  ...
  for (int c = 0; c < ci; c++) {
    for (int i = 0; i < ISIZE; i++) {
      if (j < gtoa_act_params<B>::cmax) {
        ...
        bparsc[4*c*RELU6_PAR_PER_CHAN*ISIZE+0*ISIZE+i] = iprescale;       // BRB0 prescale
        bparsh[2*c*RELU6_PAR_PER_CHAN*ISIZE+1*ISIZE+i] = fposscale.data;  // BRH1 scale
        bparsw[1*c*RELU6_PAR_PER_CHAN*ISIZE+3*ISIZE+i] = fbias.data;      // BRW3 bias
      ...
      if (o < gtoa_act_params<B>::cmax) {
        ...
        bparsh[2*c*RELU6_PAR_PER_CHAN*ISIZE+2*ISIZE+i] = ipost;           // BRH2 postscale + offset
        bparsw[1*c*RELU6_PAR_PER_CHAN*ISIZE+2*ISIZE+i] = fclipmax.data;   // BRW2 clipmax
...
template<template<typename> class B>
void gtoa_relu6_params<B>::param_enc(const tensor_t<prescale_t,1,xbuffer_t>&   prescale,    // per channel 2b+6b prescale for scaling down wide accumulators
                                  const tensor_t<int32_t,1,xbuffer_t>&      bias,        // per channel bias [Cout]
                                  const tensor_t<int16_t,1,xbuffer_t>&      posscale,    // per channel positive scaling factor [Cout]
                                  const tensor_t<uint8_t,1,xbuffer_t>&      posshift,    // per channel positive shift right amount [Cout]
                                  const tensor_t<int32_t,1,xbuffer_t>&      clipmax,     // per channel output maximum clip value [Cout]
