...
/*
 * tensor_gtoa_eltop_rescale_inline_legacy.hpp
 *
  ...
  for (int c = 0; c < ci; c++) {
    for (int i = 0; i < ISIZE; i++) {
      if (j < gtoa_eltop_rescale_params<B>::cmax) {
        ...
        bparsc[4*c*ELTOP_RESCALE_PAR_PER_CHAN*ISIZE+0*ISIZE+i] = iprescale0;      // BRB0 prescale0
        bparsc[4*c*ELTOP_RESCALE_PAR_PER_CHAN*ISIZE+1*ISIZE+i] = iprescale1;      // BRB1 prescale1
        bparsh[2*c*ELTOP_RESCALE_PAR_PER_CHAN*ISIZE+1*ISIZE+i] = fposscale.data;  // BRH1 scale
        bparsh[2*c*ELTOP_RESCALE_PAR_PER_CHAN*ISIZE+2*ISIZE+i] = ipost;           // BRH2 postscale + offset
        bparsw[1*c*ELTOP_RESCALE_PAR_PER_CHAN*ISIZE+2*ISIZE+i] = fbias.data;      // BRW2 bias
        j++;
      }
    }
  }
