...
/*
 * tensor_gtoa_eltop_plut_inline_legacy.hpp
 *
...
// constructors
// typical plut from accumulator (32b/48b) to feature-map (8b/16b)
...
// plut from accumulator (32b/48b) to accumulator (32b)
template<template<typename> class B>
gtoa_eltop_plut_params<B>::gtoa_eltop_plut_params(
                                     aoffset_t                  noi,           // number output channels (not vectors)
                                     const shape<3>&            oshpi,         // output shape, indexed by spatial_axis_t
  ...
  for (int c = 0; c < ci; c++) {
    for (int i = 0; i < ISIZE; i++) {
      if (j < gtoa_eltop_plut_params<B>::cmax) {
        ...
        bparsc[4*c*ELTOP_PLUT_PAR_PER_CHAN*ISIZE+0*ISIZE+i] = iprescale0;      // BRB0 prescale0
        bparsc[4*c*ELTOP_PLUT_PAR_PER_CHAN*ISIZE+1*ISIZE+i] = iprescale1;      // BRB1 prescale1
        bparsh[2*c*ELTOP_PLUT_PAR_PER_CHAN*ISIZE+1*ISIZE+i] = ipost;           // BRH1 postscale + offset
        bparsw[1*c*ELTOP_PLUT_PAR_PER_CHAN*ISIZE+1*ISIZE+i] = fbias.data;      // BRW1 bias
        j++;
      }
    }
  }
