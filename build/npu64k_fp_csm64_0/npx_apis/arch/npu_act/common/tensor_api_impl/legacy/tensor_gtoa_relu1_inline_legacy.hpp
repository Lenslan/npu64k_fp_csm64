...
/*
 * tensor_gtoa_relu1_inline_legacy.hpp
 *
 * File defining a tensor level relu1 API base on the generic tensor operation API
 *
 */
...
gtoa_relu1_params<B>::gtoa_relu1_params(
  ...
  gtoa_act_params<B>::set_per_channel(RELU1_PAR_PER_CHAN);
}
...
// typical relu1 from feature-map (8b/16b) to feature-map (8b/16b)
template<template<typename> class B>
gtoa_relu1_params<B>::gtoa_relu1_params(
                                     aoffset_t                  noi,           // number output channels (not vectors)
...
// relu1 from feature-map (8b/16b) to accumulator (32b)
template<template<typename> class B>
gtoa_relu1_params<B>::gtoa_relu1_params(
                                     aoffset_t                  noi,           // number output channels (not vectors)
                                     bool                       fmidbli,       // double wide input featuremap
...
// 32b input, 8b output
template<template<typename> class B>
void gtoa_relu1_params<B>::param_enc(const tensor_t<int32_t,1,xbuffer_t>&      bias,        // per channel bias [Cout]
  ...
  for (int c = 0; c < ci; c++) {
    for (int i = 0; i < ISIZE; i++) {
      if (j < gtoa_act_params<B>::cmax) {
        ...
        bparsc[4*c*RELU1_PAR_PER_CHAN*ISIZE+0*ISIZE+i] = iprescale;       // BRB0 prescale
        bparsh[2*c*RELU1_PAR_PER_CHAN*ISIZE+1*ISIZE+i] = fposscale.data;  // BRH1 scale
        bparsw[1*c*RELU1_PAR_PER_CHAN*ISIZE+3*ISIZE+i] = fbias.data;      // BRW3 bias
        j = (in_param_shape == 1 ? 0 : j + 1);
      }
      if (o < gtoa_act_params<B>::cmax) {
        int32_t    iasymm(asymm.read({o}));
        int16_t    ipost((iasymm<<8) | prescale_t().data);
        fp_e8m23_t fclipmax((float)clipmax.read({o}));

        bparsh[2*c*RELU1_PAR_PER_CHAN*ISIZE+2*ISIZE+i] = ipost;           // BRH2 postscale + offset
        bparsw[1*c*RELU1_PAR_PER_CHAN*ISIZE+2*ISIZE+i] = fclipmax.data;   // BRW2 clipmax
...
// 48b input, 8b output
template<template<typename> class B>
void gtoa_relu1_params<B>::param_enc(const tensor_t<prescale_t,1,xbuffer_t>&   prescale,    // per channel 2b+6b prescale for scaling down wide accumulators (pse
...
template<template<typename> class B>
void gtoa_relu1_params<B>::param_enc(const tensor_t<prescale_t,1,xbuffer_t>&   prescale,    // per channel 2b+6b prescale for scaling down wide accumulators
                                  const tensor_t<int32_t,1,xbuffer_t>&      bias,        // per channel bias [Cout]
                                  const tensor_t<int16_t,1,xbuffer_t>&      posscale,    // per channel positive scaling factor [Cout]
                                  const tensor_t<uint8_t,1,xbuffer_t>&      posshift,    // per channel positive shift right amount [Cout]
                                  const tensor_t<int32_t,1,xbuffer_t>&      clipmax,     // per channel output maximum clip value [Cout]
