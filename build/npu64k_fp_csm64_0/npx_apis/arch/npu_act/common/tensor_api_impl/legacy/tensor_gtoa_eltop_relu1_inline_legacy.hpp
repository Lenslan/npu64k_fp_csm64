...
/*
 * tensor_gtoa_eltop_relu1_inline_legacy.hpp
 *
 * File defining a tensor level eltwise operation fused with subsequent relu1 API based on the generic tensor operation API
 *
 */

// constructors
// typical relu1 from accumulator (32b/48b) to feature-map (8b/16b)
template<template<typename> class B>
gtoa_eltop_relu1_params<B>::gtoa_eltop_relu1_params(
  ...
  gtoa_eltop_act_params<B>::set_per_channel(ELTOP_RELU1_PAR_PER_CHAN);
}
// relu1 from accumulator (32b/48b) to accumulator (32b)
template<template<typename> class B>
gtoa_eltop_relu1_params<B>::gtoa_eltop_relu1_params(
                                     aoffset_t                  noi,           // number output channels (not vectors)
                                     const shape<3>&            oshpi,         // output shape, indexed by spatial_axis_t
...
template<template<typename> class B>
void gtoa_eltop_relu1_params<B>::param_enc(
  ...
  int ci = (gtoa_eltop_relu1_params<B>::cmax+ISIZE-1)/ISIZE;
  for (int c = 0; c < ci; c++) {
    for (int i = 0; i < ISIZE; i++) {
      if (j < gtoa_eltop_relu1_params<B>::cmax) {
        fp_e8m23_t fbias((float)bias.read({j}));
        bparsw[1*c*ELTOP_RELU1_PAR_PER_CHAN*ISIZE+3*ISIZE+i] = fbias.data;      // BRW3 bias
        bparsw[1*c*ELTOP_RELU1_PAR_PER_CHAN*ISIZE+4*ISIZE+i] = 0;               // BRW4 bias
        fp_e5m10_t fposscale(scl_fix2flt(posscale.read({j}), posshift.read({j})));
        bparsh[2*c*ELTOP_RELU1_PAR_PER_CHAN*ISIZE+1*ISIZE+i] = fposscale.data;  // BRH1 scale
        j++;
      }
      if (p0 < gtoa_eltop_relu1_params<B>::cmax) {
        ...
        bparsc[4*c*ELTOP_RELU1_PAR_PER_CHAN*ISIZE+0*ISIZE+i] = iprescale0;      // BRB0 prescale0
        p0 = (in0_param_shape == 1 ? 0 : p0 + 1);
      }
      if (p1 < gtoa_eltop_relu1_params<B>::cmax) {
        // convert legacy prescale
        ...
        bparsc[4*c*ELTOP_RELU1_PAR_PER_CHAN*ISIZE+1*ISIZE+i] = iprescale1;      // BRB1 prescale1
        p1 = (in1_param_shape == 1 ? 0 : p1 + 1);
      }
      if (o < gtoa_eltop_relu1_params<B>::cmax) {
        fp_e8m23_t fclipmax((float)clipmax.read({o}));
        bparsw[1*c*ELTOP_RELU1_PAR_PER_CHAN*ISIZE+2*ISIZE+i] = fclipmax.data;   // BRW2 clipmax
        int32_t    iasymm(asymm.read({o}));
        int16_t    ipost((iasymm<<8) | prescale_t().data);
        bparsh[2*c*ELTOP_RELU1_PAR_PER_CHAN*ISIZE+2*ISIZE+i] = ipost;           // BRH2 postscale + offset
        fp_e8m23_t post_scale = fp_e8m23_t((float)1);
        bparsw[1*c*ELTOP_RELU1_PAR_PER_CHAN*ISIZE+5*ISIZE+i] = post_scale.data;              // BRW5 output scale and shift
...
template<typename T>
void gtoa_eltop_relu1_params<B>::param_enc(
                                  const tensor_t<int16_t,1,xbuffer_t>&      prescale0,   // per channel 2b+6b prescale for scaling input accum
                                  ...
                                  const tensor_t<int8_t,1,xbuffer_t>&       preshift1,   // per channel 2b+6b preshift for scaling input feature-mapconst tensor_t<int32_t,1,xbuffer_t>&      bias0,       // per channel bias [Cout]
...
gtoa_eltop_relu1_params<B>::param_enc(prescale0,
                                      prescale1,
                                      preshift0,
                                      preshift1,
                                      bias0,
...
void gtoa_eltop_relu1_params<B>::param_enc(
  ...
  aoffset_t out_param_shape = asymm.get_dim(0);
  act_bin_op_t eltop;
  gtoa_eltop_act_params<B>::get_op(eltop);
  ...
  for (int c = 0; c < ci; c++) {
    for (int i = 0; i < ISIZE; i++) {
      ...
      if (j < gtoa_eltop_relu1_params<B>::cmax) {
        ...
        bparsh[2*c*ELTOP_RELU1_PAR_PER_CHAN*ISIZE+1*ISIZE+i] = fposscale.data;                 // BRH1 shiftp+scalep
        bparsw[1*c*ELTOP_RELU1_PAR_PER_CHAN*ISIZE+2*ISIZE+i] = fclipmax.data;                  // BRW2 clipmax
      ...
      if (p0 < gtoa_eltop_relu1_params<B>::cmax) {
        ...
        prescale_t encoded_prescale_0 = SET_PRESCALE_EXP((_prescale0), (shift_bias0 - preshift0.read({p0})));
        bparsc[4*c*ELTOP_RELU1_PAR_PER_CHAN*ISIZE+0*ISIZE+i] = encoded_prescale_0.data;                 // BRB0 prescale input accum
        p0 = (in0_param_shape == 1 ? 0 : p0 + 1);
      }
      if (p1 < gtoa_eltop_relu1_params<B>::cmax) {
        ...
        prescale_t encoded_prescale_1 = SET_PRESCALE_EXP((_prescale1), (shift_bias1 - preshift1.read({p1})));
        bparsc[4*c*ELTOP_RELU1_PAR_PER_CHAN*ISIZE+1*ISIZE+i] = encoded_prescale_1.data;                 // BRB1 prescale input featuremap
        p1 = (in1_param_shape == 1 ? 0 : p1 + 1);
      }
      if (b0 < gtoa_eltop_relu1_params<B>::cmax) {
        fp_e8m23_t fbias0_((float)bias0.read({b0}));
        bparsw[1*c*ELTOP_RELU1_PAR_PER_CHAN*ISIZE+3*ISIZE+i] = fbias0_.data;                            // BRW3 bias
        b0 = (in0_param_shape == 1 ? 0 : b0 + 1);
      }
      if (b1 < gtoa_eltop_relu1_params<B>::cmax) {
        fp_e8m23_t fbias1_((float)bias1.read({b1}));
        bparsw[1*c*ELTOP_RELU1_PAR_PER_CHAN*ISIZE+4*ISIZE+i] = fbias1_.data;                            // BRW4 bias
        b1 = (in1_param_shape == 1 ? 0 : b1 + 1);
      }
      if (o < gtoa_eltop_relu1_params<B>::cmax) {
        int32_t    iasymm(asymm.read({o}));
        int16_t    ipost((iasymm<<8) | prescale_t().data);
        bparsh[2*c*ELTOP_RELU1_PAR_PER_CHAN*ISIZE+2*ISIZE+i] = ipost;                                   // BRH2 asymm output offset
        fp_e8m23_t post_scale = fp_e8m23_t((float)1);
        if (is_out_scale) {
