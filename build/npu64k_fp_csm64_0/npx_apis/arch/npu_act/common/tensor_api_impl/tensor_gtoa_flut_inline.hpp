...
gtoa_flut_params<B>::gtoa_flut_params(
  ...
  gtoa_act_params<B>::set_per_channel(FLUT_PAR_PER_CHAN);
  poscl = poscli;
  typ = tp;
}
template<template<typename> class B>
void gtoa_flut_params<B>::init_prog(act_alay_t l) {
  act_una_op_t tp;
  switch (typ) {
  case flut_typ_exp_e:
    tp = op_exp0;
  ...
  init_flut(gtoa_params<B>::hlapi, tp, l, poscl, gtoa_act_params<B>::fp16scl);
}
...
template<template<typename> class B>
void gtoa_flut_params<B>::param_enc(const tensor_t<prescale_t,1,xbuffer_t>&   prescale,    // per channel 2b+6b prescale for scaling down wide accumulators (pse
  ...
  for (int c = 0; c < ci; c++) {
    for (int i = 0; i < ISIZE; i++) {
      if (j < gtoa_act_params<B>::cmax) {
        ...
        bparsc[4*c*FLUT_PAR_PER_CHAN*ISIZE+0*ISIZE+i] = static_cast<uint8_t>(prescale.read({j}));         // BRB0 prescale set
        bparsh[2*c*FLUT_PAR_PER_CHAN*ISIZE+1*ISIZE+i] = (int16_t)0;                         // BRH1 set to 0
        bparsh[2*c*FLUT_PAR_PER_CHAN*ISIZE+2*ISIZE+i] = sclpst;                             // BRH2 postscale
        bparsw[1*c*FLUT_PAR_PER_CHAN*ISIZE+2*ISIZE+i] = (int32_t)bias.read({j}).data;       // BRW2 bias
      j++;
      }
    }
  }
  ...
  for (int c = 0; c < ci; c++) {
    for (int i = 0; i < ISIZE; i++) {
      if (j < gtoa_act_params<B>::cmax) {
        int16_t sclpst = ((uint32_t)(post.read({j}).data)) | (((int32_t)asymm.read({j}))<<8);
        bparsc[4*c*FLUT_PAR_PER_CHAN*ISIZE+0*ISIZE+i] = static_cast<uint8_t>(prescale.read({j}));         // BRB0 prescale set
        bparsh[2*c*FLUT_PAR_PER_CHAN*ISIZE+1*ISIZE+i] = (int16_t)scale.read({j}).data;      // BRH1 scale
        bparsh[2*c*FLUT_PAR_PER_CHAN*ISIZE+2*ISIZE+i] = sclpst;                             // BRH2 postscale
        bparsw[1*c*FLUT_PAR_PER_CHAN*ISIZE+2*ISIZE+i] = (int32_t)bias.read({j}).data;       // BRW2 bias
      j++;
      }
