...
template<template<typename> class B>
void gtoa_relu1_params<B>::init_prog(act_alay_t l) {
  // initialize the PReLU microcode
  init_relu1(gtoa_params<B>::hlapi, l, gtoa_act_params<B>::fp16scl);
}
...
void gtoa_relu1_params<B>::param_enc(const tensor_t<prescale_t,1,xbuffer_t>&   prescale,    // per channel 2b+6b prescale for scaling down wide accumulators (pse
  ...
  init_relu1(gtoa_params<B>::hlapi, gtoa_params<B>::onn == 2 ? alayo32 : alayo16, gtoa_act_params<B>::fp16scl);
  ...
  for (int c = 0; c < ci; c++) {
    for (int i = 0; i < ISIZE; i++) {
      if (j < gtoa_act_params<B>::cmax) {
        ...
        bparsc[4*c*RELU1_PAR_PER_CHAN*ISIZE+0*ISIZE+i] = (int8_t)prescale.read({j}).data;    // BRB0 prescale
        bparsh[2*c*RELU1_PAR_PER_CHAN*ISIZE+1*ISIZE+i] = (int16_t)scale.read({j}).data;      // BRH1 scale
        bparsh[2*c*RELU1_PAR_PER_CHAN*ISIZE+2*ISIZE+i] = sclpst;                             // BRH2 postscale + offset
        bparsw[1*c*RELU1_PAR_PER_CHAN*ISIZE+2*ISIZE+i] = (int32_t)clipmax.read({j}).data;    // BRW2 clipmax
        bparsw[1*c*RELU1_PAR_PER_CHAN*ISIZE+3*ISIZE+i] = (int32_t)bias.read({j}).data;       // BRW3 bias
        j++;
      }
    }
  }
