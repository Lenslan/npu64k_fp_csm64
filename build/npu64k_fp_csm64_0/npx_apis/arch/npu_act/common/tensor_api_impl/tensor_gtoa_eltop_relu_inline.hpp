...
template<template<typename> class B>
void gtoa_eltop_relu_params<B>::init_prog(act_alay_t l, act_bin_op_t op, bool ea, broadcast_t brdc_f) {
  // initialize the eltwise PReLU microcode
  init_eltop_relu(gtoa_params<B>::hlapi, l, op, ea, gtoa_eltop_act_params<B>::fp16scl);
}
...
void gtoa_eltop_relu_params<B>::param_enc(
  ...
  init_eltop_relu(gtoa_params<B>::hlapi,
  ...
  for (int c = 0; c < ci; c++) {
    for (int i = 0; i < ISIZE; i++) {
      if (j < gtoa_eltop_relu_params<B>::cmax) {
        ...
        bparsc[4*c*ELTOP_RELU_PAR_PER_CHAN*ISIZE+0*ISIZE+i] = (int8_t)prescale0.read({j}).data;   // BRB0 prescale0
        bparsc[4*c*ELTOP_RELU_PAR_PER_CHAN*ISIZE+1*ISIZE+i] = (int8_t)prescale1.read({j}).data;   // BRB1 prescale1
        bparsh[2*c*ELTOP_RELU_PAR_PER_CHAN*ISIZE+1*ISIZE+i] = (int16_t)posscale.read({j}).data;   // BRH1 scale
        bparsh[2*c*ELTOP_RELU_PAR_PER_CHAN*ISIZE+2*ISIZE+i] = sclpst;                             // BRH2 postscale + offset
        bparsw[1*c*ELTOP_RELU_PAR_PER_CHAN*ISIZE+2*ISIZE+i] = (int32_t)bias.read({j}).data;       // BRW2 bias
        j++;
      }
    }
  }
