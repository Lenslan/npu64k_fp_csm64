...
/**                                                                     **/
/*************************************************************************/
#ifndef NPU_ACT_COMMON_TENSOR_API_IMPL_TENSOR_GTOA_RESCALE_INLINE_HPP_
#define NPU_ACT_COMMON_TENSOR_API_IMPL_TENSOR_GTOA_RESCALE_INLINE_HPP_
/*
 * tensor_gtoa_rescale_inline.hpp
 *
 * File defining a tensor level rescale API base on the generic tensor operation API
 *
 */
#include "./legacy/tensor_gtoa_rescale_inline_legacy.hpp"
...
template<template<typename> class B>
void gtoa_rescale_params<B>::init_prog(act_alay_t l) {
  // initialize the rescale microcode
  init_rescale(gtoa_params<B>::hlapi, l, gtoa_act_params<B>::fp16scl);
}
...
void gtoa_rescale_params<B>::param_enc(const tensor_t<prescale_t, 1, xbuffer_t>& prescale,
                                  // per channel 2b+6b prescale for scaling down wide accumulators
                                       const tensor_t<fp_e8m23_t, 1, xbuffer_t>& in_bias,
                                                                        // per channel bias [Cout]
                                       const tensor_t<fp_e5m10_t, 1, xbuffer_t>& scale,
                                                     // per channel positive scaling factor [Cout]
                                       const tensor_t<prescale_t, 1, xbuffer_t>& post,
                                                  // per channel output quantization offset [Cout]
                                       const tensor_t<int8_t, 1, xbuffer_t>& out_bias,
                                                  // outputs, buffers preallocated by caller
                                       gtoa_params_impl_pchan<xbuffer_t>& obuf ) {  // [NOLINT]
                                                                      // output encoded parameters
  gtoa_act_params<B>::fp16scl = true;
  init_rescale(gtoa_params<B>::hlapi, gtoa_params<B>::onn == 2 ? alayo32 : alayo16, gtoa_act_params<B>::fp16scl);
  ...
  for (int c = 0; c < ci; c++) {
    for (int i = 0; i < ISIZE; i++) {
      if (j < gtoa_act_params<B>::cmax) {
        int16_t sclpst = ((uint32_t)(post.read({j}).data)) | (((int32_t)out_bias.read({j}))<<8);
        // BRB0 prescale set
        bparsc[4*c*RESCALE_PAR_PER_CHAN*ISIZE+0*ISIZE+i] = prescale.read({j}).data;
        // BRH1 scale
        bparsh[2*c*RESCALE_PAR_PER_CHAN*ISIZE+1*ISIZE+i] = (int16_t)scale.read({j}).data;
        // BRH2 postscale + asymmetric offset
        bparsh[2*c*RESCALE_PAR_PER_CHAN*ISIZE+2*ISIZE+i] = sclpst;
        // BRW2 bias
        bparsw[1*c*RESCALE_PAR_PER_CHAN*ISIZE+2*ISIZE+i] = (int32_t)in_bias.read({j}).data;
      j++;
      }
    }
  }
...
void gtoa_rescale_params<B>::param_enc(const tensor_t<prescale_t, 1, xbuffer_t>& prescale,
                                  // per channel 2b+6b prescale for scaling down wide accumulators
                                       const tensor_t<fp_e8m23_t, 1, xbuffer_t>& in_bias,
                                                                        // per channel bias [Cout]
                                       const tensor_t<fp_e8m7_t, 1, xbuffer_t>& scale,
                                                     // per channel positive scaling factor [Cout]
                                       const tensor_t<prescale_t, 1, xbuffer_t>& post,
                                                  // per channel output quantization offset [Cout]
                                       const tensor_t<int8_t, 1, xbuffer_t>& out_bias,
...
#undef HLAPI
#endif  // NPU_ACT_COMMON_TENSOR_API_IMPL_TENSOR_GTOA_RESCALE_INLINE_HPP_
