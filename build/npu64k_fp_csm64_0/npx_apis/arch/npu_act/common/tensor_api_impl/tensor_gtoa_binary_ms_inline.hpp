...
/*
 * tensor_gtoa_binary_ms_inline.hpp
 *
 * File defining a tensor level binary API base on the generic tensor operation API
 *
 */
#include "./legacy/tensor_gtoa_binary_ms_inline_legacy.hpp"
...
template<template<typename> class B>
void gtoa_binary_ms_params<B>::init_bin_prog(act_bin_op_t opi, broadcast_t brdc_f) {
  bool isfp = (opi == op_fadd  || opi == op_fsub  || opi == op_frsub || opi == op_fmin
            ...
            || opi == op_flte  || opi == op_mpy   || opi == op_mpyh  || opi == op_mpyb);
  // initialize the binary_ms microcode
  if (isfp) {
    init_binary_rs_fp(HLAPI, opi);
  } else {
    init_binary_rs(HLAPI, opi);
  }
}

template<template<typename> class B>
void gtoa_binary_ms_params<B>::init_bin_sr(int32_t val) {
  HLAPI.i.u.op.scl[0] = val;
}
