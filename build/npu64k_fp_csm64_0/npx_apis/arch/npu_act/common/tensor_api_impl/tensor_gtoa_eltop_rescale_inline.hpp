...
/*
 * tensor_gtoa_eltop_rescale_inline.hpp
 *
 * File defining a tensor level eltwise operation fused with subsequent rescale API base on the generic tensor operation API
 *
 */
#include "./legacy/tensor_gtoa_eltop_rescale_inline_legacy.hpp"

// constructors
template<template<typename> class B>
gtoa_eltop_rescale_params<B>::gtoa_eltop_rescale_params(
                                     ...
                                     act_dtype_t                in1tp,         // type of secondary
  ...
  gtoa_eltop_act_params<B>::set_per_channel(ELTOP_RESCALE_PAR_PER_CHAN);
}

template<template<typename> class B>
void gtoa_eltop_rescale_params<B>::init_prog(act_alay_t l, act_bin_op_t op, bool ea, broadcast_t brdc_f) {
  // initialize the eltwise PReLU microcode
  init_eltop_rescale(gtoa_params<B>::hlapi, l, op, ea, gtoa_eltop_act_params<B>::fp16scl, brdc_f);
}
...
template<template<typename> class B>
void gtoa_eltop_rescale_params<B>::param_enc(
  ...
  init_eltop_rescale(gtoa_params<B>::hlapi,
  ...
  aoffset_t j = 0;
  int ci = (gtoa_eltop_rescale_params<B>::cmax+ISIZE-1)/ISIZE;
  for (int c = 0; c < ci; c++) {
    for (int i = 0; i < ISIZE; i++) {
      if (j < gtoa_eltop_rescale_params<B>::cmax) {
        int16_t sclpst = ((uint32_t)post.read({j}).data) | (((int32_t)asymm.read({j}))<<8);
        bparsc[4*c*ELTOP_RESCALE_PAR_PER_CHAN*ISIZE+0*ISIZE+i] = (int8_t)prescale0.read({j}).data;   // BRB0 prescale0
        bparsc[4*c*ELTOP_RESCALE_PAR_PER_CHAN*ISIZE+1*ISIZE+i] = (int8_t)prescale1.read({j}).data;   // BRB1 prescale1
        bparsh[2*c*ELTOP_RESCALE_PAR_PER_CHAN*ISIZE+1*ISIZE+i] = (int16_t)posscale.read({j}).data;   // BRH1 scale
        bparsh[2*c*ELTOP_RESCALE_PAR_PER_CHAN*ISIZE+2*ISIZE+i] = sclpst;                             // BRH2 postscale + offset
        bparsw[1*c*ELTOP_RESCALE_PAR_PER_CHAN*ISIZE+2*ISIZE+i] = (int32_t)bias.read({j}).data;       // BRW2 bias
        j++;
      }
    }
  }
