...
/*
 * tensor_gtoa_gridpad_inline.hpp
 *
 * File defining a tensor level gridpad API based on the generic tensor operation API
 *
 */

#ifndef NPU_ACT_COMMON_TENSOR_API_IMPL_TENSOR_GTOA_GRIDPAD_INLINE_HPP_
#define NPU_ACT_COMMON_TENSOR_API_IMPL_TENSOR_GTOA_GRIDPAD_INLINE_HPP_

#define HLAPI gtoa_params<B>::hlapi

// constructor
// gridpad operation from grid (float 16b) to grid (float 16b)
template<template<typename> class B>
gtoa_gridpad_params<B>::gtoa_gridpad_params(
                 act_pad_t        pad_type,      // padding type (zero, border, mirror, replic)
  ...
  aoffset_t wchunk = (pad_type == pad_mirror || pad_type == pad_replic) ? 1 : NRDIV_UP(wsize, NRDIV_UP(wsize, TNSVSIZE));
  aoffset_t chunks = NRDIV_UP(wsize, wchunk);

  // input index in ixpix_t [1][D2][D1][D0][2] format
  shapes.ishape = {2,                            // x&y
                   idxshpi[0],                   // D0 (FSw*FSh)
                   //wsize,                        // D1/TNSVSIZE (W_out)
  ...
  // init microcode
  init_grid_pad(HLAPI, pad_type, (float)(dictshpi[0]-1), (float)(dictshpi[1]-1));
...
void gtoa_gridpad_params<B>::init_l1_output(
  // output tensor in L1 memory [1][D2][D1/8][D0][2]
...
template <template <typename> class B>
template <template <typename> class BD>
void gtoa_gridpad_params<B>::init_l1_input0(
...
template<template<typename> class B>
inline void gtoa_gridpad_params<B>::get_shapes(
...
#endif  // NPU_ACT_COMMON_TENSOR_API_IMPL_TENSOR_GTOA_GRIDPAD_INLINE_HPP_
