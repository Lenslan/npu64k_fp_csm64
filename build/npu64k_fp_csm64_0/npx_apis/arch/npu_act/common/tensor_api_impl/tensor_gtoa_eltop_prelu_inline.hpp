...
template<template<typename> class B>
void gtoa_eltop_prelu_params<B>::init_prog(act_alay_t l, act_bin_op_t op, bool ea, broadcast_t brdc_f) {
  // initialize the eltwise PReLU microcode
  init_eltop_prelu(gtoa_params<B>::hlapi, l, op, ea, gtoa_eltop_act_params<B>::fp16scl, brdc_f);
}
...
void gtoa_eltop_prelu_params<B>::param_enc(
  ...
  init_eltop_prelu(gtoa_params<B>::hlapi,
  ...
  for (int c = 0; c < ci; c++) {
    for (int i = 0; i < ISIZE; i++) {
      if (j < gtoa_eltop_prelu_params<B>::cmax) {
        ...
        bparsc[4*c*ELTOP_PRELU_PAR_PER_CHAN*ISIZE+0*ISIZE+i] = prescale0.read({j}).data;           // BRB0 prescale input accum
        bparsc[4*c*ELTOP_PRELU_PAR_PER_CHAN*ISIZE+1*ISIZE+i] = prescale1.read({j}).data;           // BRB1 prescale input featuremap
        bparsh[2*c*ELTOP_PRELU_PAR_PER_CHAN*ISIZE+1*ISIZE+i] = sclpst;                             // BRH1 postscale + offset
        bparsw[1*c*ELTOP_PRELU_PAR_PER_CHAN*ISIZE+1*ISIZE+i] = ((uint32_t)posscale.read({j}).data<<16)|
          negscale.read({j}).data;                                                                 // BRW1 positive/negative scaling
        bparsw[1*c*ELTOP_PRELU_PAR_PER_CHAN*ISIZE+2*ISIZE+i] = bias.read({j}).data;                // BRW2 bias
        j++;
      }
    }
  }
