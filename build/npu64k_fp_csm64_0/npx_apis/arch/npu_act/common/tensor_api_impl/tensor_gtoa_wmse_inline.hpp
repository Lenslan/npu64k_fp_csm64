...
/*
 * tensor_gtoa_wmse_inline.hpp
 *
 * File defining a tensor level w dim wise mean square error API base on the generic tensor operation API
 *
 */
#ifndef NPU_ACT_COMMON_TENSOR_API_IMPL_TENSOR_GTOA_WMSE_INLINE_HPP_
#define NPU_ACT_COMMON_TENSOR_API_IMPL_TENSOR_GTOA_WMSE_INLINE_HPP_
...
gtoa_wmse_params<B>::gtoa_wmse_params(
  ...
  init_wmse(HLAPI, useAcc, keepAcc);
  set_wmse_params();
...
// set iterators
template <template <typename> class B>
void gtoa_wmse_params<B>::set_wmse_params() {
...
void gtoa_wmse_params<B>::init_l1_input(
...
template<template<typename> class B>
template<template<typename> class BD>
void gtoa_wmse_params<B>::init_l1_output(
...
template <template <typename> class B>
inline void gtoa_wmse_params<B>::get_shapes(
...
template <template <typename> class B>
inline void gtoa_wmse_params<B>::modif_func(bool keepAcc, gtoa_params_impl_modif& m) {  // NOLINT [runtime/references]
  act_hlapi_t h;
  init_wmse(h, false, keepAcc);
  m = h.i.u.op.act_prog;
}
...
inline void gtoa_wmse_params<B>::modif_out_fm(
  ...
  impl_spec_container_t<BD> p(nullptr, {(aoffset_t)(fmodbli ? 2 * c : c), 1, (aoffset_t)TNSVSIZE, 1, 1}, use_opt_vm_layout);
  gtoa_params<B>::tens = *reinterpret_cast<const impl_spec_container_t<B>*>(&p);
  // [D][H][W][Grp][C] --> [D][H][W][Grp][C/mlsb][mlsb]
...
template <template <typename> class B>
inline void gtoa_wmse_params<B>::modif_out_acc(
  aoffset_t                 noi,  // number output channels
...
template <template <typename> class B>
void gtoa_wmse_params<B>::init_wmse_scale(
...
#endif  // NPU_ACT_COMMON_TENSOR_API_IMPL_TENSOR_GTOA_WMSE_INLINE_HPP_
